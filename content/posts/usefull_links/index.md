---
title: Useful links
description: useful links for learning web dev and marketing tools
date: '2020-05-18'
draft: false
slug: '/blog/usefull-links'
tags:
  - WordPress
---

## WordPress:
[WordPress Codex](https://codex.wordpress.org/ 'WordPress Codex')  
[WordPress Plugins](https://wordpress.org/support/article/plugins/ 'WordPress Plugins')  
[WordPress Themes](https://https://wordpress.org/themes/ 'WordPress Themes')  
[Salient Theme Documentation](http://themenectar.com/docs/salient/ 'Salient theme Documentation')  

## ThemeForest
[Search for themes](https://themeforest.net/popular_item/by_category?category=wordpress 'Themeforest Themes')

## WooCommerce
[Get started with WooCommerce](https://docs.woocommerce.com/documentation/plugins/woocommerce/getting-started/ 'How to get started with woocommerce')  

## Mailchimp
[Mailchimp help](https://mailchimp.com/help/ 'Mailchimp help')  

## Google:
[Google Analyitics Link 1](https://support.google.com/analytics/?hl=en#topic=3544906 'Google Analyitics link1')  
[Google Analyitics Link 2](https://developers.google.com/analytics 'Google Analyitics link2')  

## Facebook:
[Facebook Business link 1](https://www.facebook.com/business/help/952192354843755?id=1205376682832142 'Facebook Business link1')  
[Facebook Business link 2](https://www.facebook.com/business/help/113163272211510?id=180505742745347 'Facebook Business link2')  

## About domain:
[How domains work](https://www.wpbeginner.com/beginners-guide/beginners-guide-what-is-a-domain-name-and-how-do-domains-work/ 'How domains work')  

## About DNS:
[How DNS work](https://www.cloudflare.com/learning/dns/what-is-dns/ 'How DNS work') 

# If you have any questions feel free to ask us directly on the following emails:
[Tomislav: tomislav@tomislavbalabanov.me](mailto:tomislavbalabanov.me 'Send mail to Tomislav')  
[Emir: e@emiralkafadji.com](mailto:e@emiralkafadji.com 'Send mail to Emir')

